...
def generate_heatmap_k_vs_q(csv_path, output_dir):
    ...
    grouped = df.groupby(['Q_Budget', 'K_Value'])['Cost_Ratio'].mean().reset_index()
    
    # Pivot the data into a 2D matrix format for the heatmap
    pivot_table = grouped.pivot(index='Q_Budget', columns='K_Value', values='Cost_Ratio')
    ...
    plt.title('Budget Starvation: Coreset Size vs. Model Complexity')
    
    os.makedirs(output_dir, exist_ok=True)
    out_file = os.path.join(output_dir, "heatmap_k_vs_q.png")
...
if __name__ == "__main__":
    ...
    if os.path.exists(metrics_file):
        generate_heatmap_k_vs_q(metrics_file, output_directory)
    else:
        print(f"Error: {metrics_file} not found. Run the master pipeline first.")